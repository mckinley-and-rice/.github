...
이 저장소는 자기가 발행하는 표준을 스스로 따르며, 의도적인 예외가 하나 있습니다. 변경은
`develop` 을 향한 풀 리퀘스트로 들어옵니다.

| | |
|---|---|
| `main` | **기본.** 발행된 상태. GitHub가 상속되는 health file을 여기서 읽습니다. |
| `develop` | 통합. 모든 변경이 먼저 여기로 들어오고, 그 뒤 승격됩니다. |
| 브랜치 이름 | `<type>/<slug>`, 타입은 `feat fix chore docs test refactor perf`. |
| 필수 체크 | `branch name follows the convention` |

두 브랜치 모두 보호됩니다. 풀 리퀘스트만 허용되고, force push와 삭제는 막으며, admin 강제가
켜져 있습니다. 그래서 보호가 여기서 일하는 유일한 사람들에게 권고로 남지 않고 메인테이너에게도
구속력을 갖습니다.

### 표준은 `develop` 이라고 하는데 여기서는 왜 `main` 이 기본인가

[docs/GITFLOW.ko.md](./docs/GITFLOW.ko.md) 는 `develop` 을 기본 브랜치로 삼고, 그 이유는 저장소의
기본 브랜치를 읽는 사람이 기여자이며 최종 사용자는 클론하지 않고 릴리스 산출물을 설치한다는
것입니다.

`.github` 저장소에서는 그 이유가 뒤집힙니다. **GitHub는 community health file을 기본 브랜치에서
읽습니다.** 추측이 아니라 측정한 사실입니다. `develop` 이 기본일 때 이 조직의 모든 저장소가
상속되는 행동 규범을 `/blob/develop/CODE_OF_CONDUCT.md` 로 해석했습니다. 그러니 여기서 기본
브랜치는 기여자의 시야가 아니라 241개 저장소에 제공되는 표면이고, 그것을 통합 브랜치로 향하게 하면
한 시간 전에 머지된 것이 그대로 발행됩니다.

그래서 `main` 이 기본이고, `develop` 은 여전히 통합합니다. 이 답을 만들어 낸 것이 표준 자신의
근거이므로, 이 예외는 표준을 깨는 것이 아니라 표준을 따르는 것입니다. 여기에 적어 두는 이유는,
기록되지 않은 예외는 실수로 읽히고 다음 사람이 그것을 "고쳐" 되돌리기 때문입니다.

### 이 모델의 비용

`main` 은 `develop` 에서 오는 승격 풀 리퀘스트로 움직이며 **merge 커밋**으로 머지되고, 그 승격 뒤에
`main` 을 `develop` 으로 되머지하는 풀 리퀘스트가 따라옵니다. 승격 한 번에 풀 리퀘스트 두 개가 실제
비용이고, 이 저장소는 241개의 다른 저장소에 요구하는 규칙에서 자기를 빼는 대신 그 비용을 냅니다.
...
그 상태를 남깁니다. 여기서 첫 승격에 실측한 값: `git rev-list --count origin/develop..origin/main`
이 1이었고 `main` 의 실행이 실패했으며, 되머지가 들어온 뒤 0이 되고 통과했습니다.
...
`redrob-labs` 는 자기 [`.github`](https://github.com/redrob-labs/.github) 저장소로 health file을
공급합니다. `docs/` 의 네 문서를 복제하지 않고 링크합니다. 두 조직이 같은 표준을 따르고 사본은
갈라지기 때문입니다. 그곳이 추가하는 것은 그곳에만 적용되는 것,
[FORKS.md](https://github.com/redrob-labs/.github/blob/main/FORKS.md) 입니다. 그 조직에서 포크인
저장소 여덟 개를 위한 규칙입니다.
